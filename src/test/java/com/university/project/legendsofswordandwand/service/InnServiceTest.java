...
class InnServiceTest {
  ...
  void reviveAndHealParty_whenPartyNotFound_shouldThrowException() {
    ...
    doThrow(new RuntimeException("Party not found."))
        .when(partyService)
        .reviveAndHealParty(campaignId);

    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> innService.loadInnView(campaignId));
  ...
  void purchaseItem_shouldReturnTrue() {
    ...
    when(inventoryService.purchaseItem(campaignId, itemId)).thenReturn(true);
