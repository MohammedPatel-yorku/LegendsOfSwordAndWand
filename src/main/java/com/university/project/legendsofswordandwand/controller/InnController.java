...
/**
 * MVC controller handling inn-related requests, including healing, shopping,
 * hero recruitment, and continuing the campaign after an inn visit.
 */
@Controller
...
public class InnController {
     ...
     *
     * <p>Access is permitted if the player is retreating after a loss, if an inn room
     * is pending, or if recruit data is already stored in the session. On the first
     * arrival, the party is healed and recruits are generated and cached in the session
     * by ID to avoid stale entity references on subsequent redirects. Redirects to
     * {@code /campaign} if access conditions are not met or if an error occurs.
     *
     ...
     *
     * <p>Adds a success or error flash attribute depending on whether the party had
     * sufficient gold, then redirects back to the inn.
     *
     * @param authentication     the current user's authentication
     ...
     *
     * <p>Temporary recruits are removed from the party. Session attributes for recruits,
     * heal summary, and last battle result are cleared. If the player is continuing after
     * a loss, the pending room is intentionally kept so they retry the same battle room.
     * Otherwise, the pending room is cleared normally.
     *
