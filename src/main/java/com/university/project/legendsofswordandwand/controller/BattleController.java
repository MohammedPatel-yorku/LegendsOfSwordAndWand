...
/**
 * MVC controller handling all battle-related HTTP requests, including battle
 * initialisation, player actions, result display, and campaign progression.
 */
@Controller
...
public class BattleController {
     ...
     *
     * <p>If no active {@link BattleState} exists in the session, a new battle is created for
     * the player's active campaign. Enemy turns are automatically processed until it becomes
     * the player's turn. Redirects to {@code /campaign} if initialisation fails.
     *
     ...
     * @param authentication the current user's authentication
     * @param session        the current {@link HttpSession}
     * @return {@code 200 OK} with the state DTO, {@code 401} if unauthenticated,
     *         or {@code 404} if no battle is in session
     */
    @GetMapping("/state")
    @ResponseBody
     ...
     * processes consecutive enemy turns until it is the player's turn again or the battle ends.
     * The updated state is always saved back to the session.
     *
     * @param authentication the current user's authentication
     * @param session        the current {@link HttpSession}
     ...
     * @return {@code 200 OK} with the updated state DTO, {@code 400} on invalid action,
     *         or {@code 401} if unauthenticated
     */
    @PostMapping("/action")
    @ResponseBody
     ...
     *
     * <p>On a player win, gold and experience rewards are distributed once (guarded by a
     * session flag), and any heroes with a pending level-up are identified for the view.
     * On a player loss, loss penalties are applied once. Redirects to {@code /battle} if
  ...
  public String result(Authentication authentication, Model model, HttpSession session) {
     ...
     *
     * <p>Clears the battle state and rewards flag from the session. If the campaign is
     * complete, finalises it and redirects to the completion page. If the player lost and
     * has visited the inn, redirects there for recovery. Otherwise, clears the pending
     * room and redirects to the campaign map.
     *
     * @param authentication the current user's authentication
     * @param session        the current {@link HttpSession}
