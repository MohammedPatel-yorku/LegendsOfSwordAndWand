...
class BattleServiceImpl implements IBattleService {
    ...
    /**
     * Initialises a PvE battle for the given campaign, generating a scaled enemy party
     * and building the initial turn queue.
     *
     * <p>Temporary (unchosen recruit) heroes in the party are excluded from the battle.
     * Enemy units are assigned negative battle IDs to avoid collisions with player IDs.
     *
     ...
     *
     * <p>If the active unit is stunned, the stun is ticked and the turn is skipped.
     * Supported actions are {@code ATTACK}, {@code DEFEND}, {@code WAIT}, and {@code CAST}.
     * The battle status is checked after each action and the turn is advanced if the
     ...
     *
     * <p>If the active unit is stunned, the stun is ticked and the turn is skipped.
     * If no living player targets exist, the enemy defends instead. Enemy behaviour
     * is determined by {@link #decideEnemyAction}.
     *
     * @param state the current {@link BattleState}
     * @return the updated {@link BattleState}
     */
     ...
     *
     * <ul>
     *   <li><b>Skeleton, Witch, Shadow</b> (glass cannon) — always attack the weakest target</li>
     *   <li><b>Orc, Dark Knight</b> (brute) — 60% attack highest attack hero, 40% random target</li>
     *   <li><b>Goblin, Vampire</b> (swift) — 75% attack lowest defense hero, 25% wait</li>
     *   <li><b>Troll</b> (tank) — defends if heavily hurt, otherwise targets highest HP or random</li>
     *   <li><b>Bandit, Wyvern</b> (balanced) — 85% random attack, 15% defend</li>
     * </ul>
     *
     ...
     *
     * @param state the current {@link BattleState}
     * @return {@code PLAYER_WIN} if all enemies are dead, {@code PLAYER_LOSE} if all
     *         player heroes are dead, or {@code IN_PROGRESS} otherwise
     */
    @Override
     ...
     *
     * <p>Total XP is split evenly among living heroes, with any remainder awarded to the
     * first hero. Gold is calculated from enemy levels and added to the party treasury.
     * Each surviving hero's HP and mana snapshots are written back to the database.
     *
     * @param state the current {@link BattleState}
     ...
     *
     * <p>Each hero loses 30% of the XP accumulated within their current level, floored at
     * the start of that level. HP and mana snapshots are written back to the database.
     * The party also loses 10% of their current gold.
     *
     * @param state the current {@link BattleState}
     */
    @Override
     ...
     * triggering any hybrid class passive effects.
     *
     * <p>{@code ROGUE} hybrids have a chance to apply a sneak attack bonus.
     * {@code WARLOCK} hybrids apply a mana burn to the defender on every attack.
     *
     ...
     *
     * <p>Stuns are ticked when the queue is refilled at the start of a new round.
     * The active unit and player turn flag are updated accordingly.
     *
     * @param state the current {@link BattleState}
     * @return the updated {@link BattleState}
     */
    ...
    /**
     * Clears and rebuilds the turn queue from all living units, ordered by level descending,
     * then by attack descending as a tiebreaker.
     *
