...
class InventoryServiceImpl implements IInventoryService {
    ...
    /**
     * Returns all {@link Item}s currently held in the active party's inventory for the given campaign.
     *
     * <p>Item IDs that no longer resolve to a valid item are silently filtered out.
     * Returns an empty list if the party has no inventory.
     *
     ...
     *
     * <p>If the item is a revival item, the hero is restored to full HP and mana regardless
     * of their current state. Otherwise, the hero must be alive, and their HP and mana are
     * restored by the item's respective values, capped at their maximums. One instance of
     ...
     * @throws RuntimeException if the item is not in the inventory, the hero is not found,
     *                          the item is not found, or the hero is dead and the item does not revive
     */
    @Override
