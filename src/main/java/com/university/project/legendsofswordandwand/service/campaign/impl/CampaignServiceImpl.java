...
class CampaignServiceImpl implements ICampaignService {
   ...
   *
   * <p>All permanent heroes are restored to full HP and mana before saving. The campaign is then
   * deactivated.
   *
   * @param campaignId the ID of the campaign whose party to save
   * @param userId the ID of the user saving the party
   * @throws RuntimeException if the user or campaign is not found, or if the user already has 5
   *     saved parties
   */
  @Override
   ...
   *
   * <p>The specified saved party is deleted before saving the new one via {@link
   * #savePartyFromCampaign}.
   *
   * @param campaignId the ID of the campaign whose party to save
   ...
   *
   * <p>The score is the sum of the party's base score and a bonus from any remaining inventory
   * items (each worth half their purchase cost multiplied by 10). Temporary heroes are deleted and
