...
public final class AbilityHelper {
   ...
   *
   * <p>If the target has an active shield, incoming damage is first absorbed by the shield. Any
   * damage exceeding the shield's remaining value is applied directly to the target's health. If
   * the shield is a fire shield (indicated by a negative shield value in {@link BattleState}), 10%
   * of the absorbed damage is reflected back to the attacker.
   *
   * @param attacker the {@link HeroSnapshot} of the attacking hero, used for fire shield
   *     reflection; may be {@code null} to skip reflection
