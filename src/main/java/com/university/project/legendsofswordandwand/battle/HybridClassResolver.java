...
/**
 * Spring component responsible for resolving the {@link HybridClass} that results
 * from combining two {@link HeroClass} values.
 *
 * <p>When both classes are the same, a pure specialisation is returned. When the
 * classes differ, the combination maps to a unique hybrid archetype.
 */
@Component
public class HybridClassResolver {
     ...
     *
     * <p>The resolution is order-independent: {@code resolve(ORDER, CHAOS)} and
     * {@code resolve(CHAOS, ORDER)} return the same result. If both classes are identical,
     * the pure specialisation for that class is returned:
     * <ul>
     *   <li>{@code ORDER + ORDER} → {@code PRIEST}</li>
     *   <li>{@code CHAOS + CHAOS} → {@code INVOKER}</li>
     *   <li>{@code WARRIOR + WARRIOR} → {@code KNIGHT}</li>
     *   <li>{@code MAGE + MAGE} → {@code WIZARD}</li>
     * </ul>
     *
     * Cross-class combinations resolve as follows:
     * <ul>
     *   <li>{@code ORDER + CHAOS} → {@code HERETIC}</li>
     *   <li>{@code ORDER + WARRIOR} → {@code PALADIN}</li>
     *   <li>{@code ORDER + MAGE} → {@code PROPHET}</li>
     *   <li>{@code CHAOS + WARRIOR} → {@code ROGUE}</li>
     *   <li>{@code CHAOS + MAGE} → {@code SORCERER}</li>
     *   <li>{@code WARRIOR + MAGE} → {@code WARLOCK}</li>
     * </ul>
     *
