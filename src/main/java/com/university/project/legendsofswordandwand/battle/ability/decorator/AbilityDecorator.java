 ...
 *
 * <p>Wraps an existing {@link Ability} and delegates all calls to it by default.
 * Concrete subclasses override {@link #execute} to add behaviour before or after
 * the wrapped ability fires, without modifying the wrapped class itself.
 *
 * <p>Because {@code AbilityDecorator} implements {@link Ability}, decorators are
 * fully interchangeable with base abilities and can be stacked arbitrarily.
 */
public abstract class AbilityDecorator implements Ability {
     ...
     *
     * <p>Subclasses should call {@code super.execute(...)} or {@code wrapped.execute(...)}
     * at the appropriate point to preserve the wrapped ability's behaviour.
     *
