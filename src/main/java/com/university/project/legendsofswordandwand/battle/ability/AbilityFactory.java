...
/**
 * Factory responsible for constructing {@link Ability} instances based on a hero's
 * {@link HeroClass}, {@link HybridClass}, and ability slot index.
 *
...
public class AbilityFactory {
     ...
     *
     * <ul>
     *   <li>{@code ORDER} — delegates to {@link #resolveOrder(HybridClass, int)}</li>
     *   <li>{@code CHAOS} — delegates to {@link #resolveChaos(HybridClass, int)}</li>
     *   <li>{@code WARRIOR} — always returns a {@link BerserkerAbility}</li>
     *   <li>{@code MAGE} — always returns a {@link ReplenishAbility}</li>
     * </ul>
     *
     ...
     * @param abilityIndex the ability slot index ({@code 0} for the first ability, {@code 1} for the second)
     ...
     *
     * <p>For ability slot {@code 0}: returns {@link FireShieldAbility} if the hybrid class is
     * {@code HERETIC}, otherwise returns {@link ProtectAbility}. For all other slots,
     * returns {@link HealAbility}.
     *
     ...
     *
     * <p>Returns {@link FireballAbility} for ability slot {@code 0},
     * or {@link ChainLightningAbility} for all other slots.
     *
     * @param hybridClass  the hero's hybrid class
     * @param abilityIndex the ability slot index
