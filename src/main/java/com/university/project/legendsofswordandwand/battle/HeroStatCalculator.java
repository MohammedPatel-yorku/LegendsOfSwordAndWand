...
public class HeroStatCalculator {
    ...
    /**
     * Returns the XP increment required to advance from level {@code level - 1} to {@code level}.
     *
     * <p>Formula: {@code 500 + 75 * level + 20 * level²}. Made public so that callers such as
     * {@code BattleServiceImpl} and {@code InnServiceImpl} can use it directly instead of inlining
     * their own copies of the formula (Smell 3 refactoring).
     *
     * @param level the target level (must be >= 1)
     * @return the XP step required to reach {@code level}
     */
     ...
     *
     * <p>The threshold grows quadratically: each level adds {@code 500 + 75*level + 20*level²} to
     * the previous threshold. Delegates to {@link #getExpStepForLevel(int)} to avoid duplicating
     * the formula.
     *
    ...
    private int calculateExpThreshold(int level) {
        if (level <= 1) return getExpStepForLevel(1); // Exp(1) = 500 + 75 + 20 = 595
