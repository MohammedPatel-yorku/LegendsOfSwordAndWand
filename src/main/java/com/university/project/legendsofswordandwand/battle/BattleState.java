 ...
 *
 * <p>Holds all units, the turn queue, battle log, shields, stun tracking, and overall battle
 * status. Implements {@link Serializable} to support session persistence.
 */
@Getter
@Setter
public class BattleState implements Serializable {
  ...
  /**
   * Maps each unit's battle ID to their current shield value. Negative values indicate a fire
   * shield.
   */
  ...
  /**
   * Finds and returns the {@link BattleUnit} with the given battle ID, searching both player and
   * enemy unit lists.
   *
  ...
  /**
   * Decrements the stun counter for all stunned units by one, floored at {@code 0}. Should be
  ...
  /**
   * Returns the current shield value for the unit with the given battle ID. A negative value
   * indicates a fire shield.
   *
  ...
  /**
   * Sets the shield value for the unit with the given battle ID. Use a negative value to indicate a
   * fire shield.
   *
   * @param battleId the battle ID of the unit
