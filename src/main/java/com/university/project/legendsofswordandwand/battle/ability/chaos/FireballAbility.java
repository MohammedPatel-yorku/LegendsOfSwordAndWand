 ...
 *
 * <p>If the caster's {@link HybridClass} is {@code SORCERER}, all hits deal
 * double damage. Otherwise the standard calculated damage is applied.
 */
public class FireballAbility implements Ability {
     ...
     *
     * <p>If {@code target} is {@code null}, the first three enemies in the
     * {@code enemies} list are hit. Otherwise, {@code target} is hit first,
     * followed by up to two additional enemies from the list. The damage
     * multiplier is {@code 2.0} for {@code SORCERER} and {@code 1.0} otherwise.
     * Each hit is logged to the {@link BattleState}.
     *
     * @param caster  the {@link BattleUnit} casting the ability
     * @param target  the primary {@link BattleUnit} to hit, or {@code null} to hit the first three enemies
